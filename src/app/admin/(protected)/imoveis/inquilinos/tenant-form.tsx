...
export default function TenantForm({ initial }: { initial?: any }) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-5 max-w-3xl">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <Link href="/admin/inquilinos" className="text-sm text-ink-soft hover:text-terra flex items-center gap-1.5"><ArrowLeft size={14} /> Voltar</Link>
      ...
      <div className="card-base p-6 space-y-4">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div className="sm:col-span-2">
            <label className="field-label">Endereço atual</label>
          ...
          <div className="sm:col-span-2">
            ...
            <input className="field-input" value={form.emergency_contact} onChange={e => u('emergency_contact', e.target.value)} placeholder="Nome e telefone do contato de emergência" />
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-end gap-2">
        <Link href="/admin/inquilinos" className="btn-outline">Cancelar</Link>
        <button type="submit" disabled={loading} className="btn-primary">
          {loading ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />}
          {isEdit ? 'Salvar' : 'Cadastrar'}
        </button>
